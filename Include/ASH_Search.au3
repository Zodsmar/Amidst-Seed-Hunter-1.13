...
Func EvaluateSeed($seed, $criteriaArray, $listArray, $include)
   ...
   ; Checks for black on the screen (This makes sure Amidst is fully loaded before hand
   If $timeToWait < 500 Then
	  While 1
		 Dim $checkingBlack = PixelSearch($windowDims[1] + 10, $windowDims[2], $windowDims[3] - 10, $windowDims[4], 0x000000, 5, 1, $windowDims[0])
		 If @error = 0 Then
			 ;DebugWrite("CheckingBlack" & $checkingBlack[0] & "   " & $checkingBlack[1])
		 EndIf
   ...
   ;Because Amidst fades in this sleep gives it time to make sure the colors are full for searching
   Sleep(100)
