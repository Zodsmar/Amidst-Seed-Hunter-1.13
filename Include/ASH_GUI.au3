...
Func DoBuildOptionsTab()
   ...
   $numSeedsToEvaluateLabel = GUICtrlCreateLabel("Time to Wait: (Set this to below 500 to search based if Amidst is loaded)", 20, 400)
	GUICtrlSetBkColor($numSeedsToEvaluateLabel, $GUI_BKCOLOR_TRANSPARENT)
	_ArrayAdd($optionsTabControlsArray, $numSeedsToEvaluateLabel)

    $timeToWaitGUI = GUICtrlCreateInput($timeToWait, 395, 400, 80, 20)
	_ArrayAdd($optionsTabControlsArray, $timeToWaitGUI)
...
Func DoGetResultsOutputFile()
 ...
 Func DoUpdateWaitTime()
	   $timeToWait = GUICtrlRead($timeToWaitGUI)
	   IniWrite($settingsINIFile, "Options", "TimeToWait", $timeToWait)
	   UpdateGlobals()
EndFunc
...
Func DoUpdateOptionsTabFromINIFile()
	...
	GUICtrlSetData($timeToWaitGUI, $timeToWait)
...
Func DoResetINIFileToDefaults($prompt = True)
	...
	If $response = $IDYES Or $prompt = False Then
		...
		IniWrite($settingsINIFile, "Options", "TimeToWait", 5000)
	EndIf
EndFunc
